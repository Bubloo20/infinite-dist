...
const unpack = (v: string | null | undefined): string[] => {
  if (!v) return [];
  try { const p = JSON.parse(v); return Array.isArray(p) ? p.filter(Boolean) : [v]; } catch { return [v]; }
};

/** "2026-08-16T09:00:00.000Z" -> "2026-08-16T09:00" for datetime-local. */
const forInput = (v: string | null | undefined) => (v ? String(v).slice(0, 16) : "");

/** Local "now", in the shape datetime-local wants. */
const nowLocal = () => {
  const d = new Date();
...
export default function WorkLogForm({ onDone, job, mine, editing, onCancel, signedDate }: {
  onDone?: () => void;
  job?: ClientJob | null;
  mine?: JobAssignment | null;
  /** Reopening a shift that was already submitted. */
  editing?: EditableLog | null;
  onCancel?: () => void;
  /** When they signed — the work started from there. */
  signedDate?: string | null;
}) {
  const jobRef = job ? job.job_number || `JOB-${job.id}` : "";
  const [jobNumber, setJobNumber] = useState(jobRef);
  const [startedAt, setStartedAt] = useState(
    (editing?.startedAt || signedDate || "").slice(0, 10),
  );
  const [endedAt, setEndedAt] = useState((editing?.endedAt || (editing ? "" : nowLocal())).slice(0, 10));
  const [manualTime, setManualTime] = useState(editing?.timeSpent || "");
  const [leafletCount, setLeafletCount] = useState(
    editing?.leaflets != null ? String(editing.leaflets) : (mine?.leaflet_share != null ? String(mine.leaflet_share) : ""),
  );
  const [areaWorked, setAreaWorked] = useState(editing?.areaWorked ?? (job ? mine?.area_note || job.area || "" : ""));
      ...
      timers.push(
        setTimeout(async () => {
          try {
            ...
            setChecks((c) => ({ ...c, [i]: { status: d.status, message: d.message, helpUrl: d.helpUrl, helpLabel: d.helpLabel } }));
          } catch {
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(editing ? "/api/portal/me/log" : "/api/portal/submit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      const emailed = editing ? true : await submitForm(
        {
          Worker: data.workerName || "—",
          "Job number": jobNumber,
          Area: areaWorked || "—",
          Started: new Date(startedAt).toLocaleDateString("en-AU"),
          Finished: new Date(endedAt).toLocaleDateString("en-AU"),
    ...
    return (
      <GlassCard className="border-emerald-400/25 bg-emerald-500/[0.06] p-6">
        ...
        <p className="mt-1.5 text-sm text-emerald-100/70">
          {done.stored
            ? "Saved against this job — it'll show in your earnings, and you can edit it until it's paid."
      ...
      <GlassCard className="p-10 text-center sm:p-14">
        ...
        <div className="mx-auto mt-7 flex max-w-sm flex-col gap-2 text-left text-sm">
          <span className="flex items-center gap-2.5 rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3 text-white/70">
            ...
            {done.emailed ? "Sent through" : "Email didn’t send, but your entry is saved"}
          </span>
          <span className="flex items-center gap-2.5 rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3 text-white/70">
            <span className={done.stored ? "text-emerald-400" : "text-amber-400"}>●</span>
            {done.stored ? "Saved — it'll show in your earnings" : "Not saved"}
          </span>
        </div>
      ...
      <GlassCard className={job ? "p-6 sm:p-7" : "p-7 sm:p-10"}>
        {job && (
          <div className="mb-6">
            ...
            <h3 className="mt-2 font-display text-xl font-extrabold tracking-tight text-white">
              {editing ? "Edit this shift" : "Mark this work as done"}
            </h3>
            <p className="mt-1.5 text-[14px] text-white/50">
              {editing
                ? "Change anything that isn't right and submit it again. You can keep editing until it's paid."
                : "Fill in the times you actually worked, how many leaflets went out and your tracking links. Add one for each time you go out."}
            </p>
          </div>
        )}
        <div className="grid gap-6 sm:grid-cols-2">
          <Field label="Job number" hint={job ? undefined : "Optional."}>
          ...
          <Field label="Area worked">
            <input className={`${inputCls} ${job ? "cursor-not-allowed opacity-60" : ""}`} value={areaWorked}
              onChange={(e) => setAreaWorked(e.target.value)} placeholder="e.g. Northcote — High St"
                    ...
                    <AnimatePresence>
                      {c && c.status !== "idle" && (
                        <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                          ...
                          {c.message}
                          {c.helpUrl && (
                            <a href={c.helpUrl} target="_blank" rel="noreferrer"
                              className="mt-1.5 block font-bold underline underline-offset-2 hover:opacity-80">
                              {c.helpLabel || "Watch how to fix this"} ↗
                            </a>
                          )}
                          {(c.status === "valid" || c.status === "unverified") && (
                            <a href="https://www.youtube.com/watch?v=eAUvZYUVOOQ" target="_blank" rel="noreferrer"
                              className="mt-1.5 block text-white/45 underline underline-offset-2 hover:text-white">
                              Parts of your route hidden? Here&apos;s how to fix that ↗
                            </a>
                          )}
                        </motion.div>
                      )}
        ...
        <div className="mt-8 flex flex-wrap items-center gap-4">
          <button type="submit" disabled={busy || blocked}
            className={`${busy ? "idp-loading" : ""} flex items-center gap-2.5 rounded-2xl bg-gradient-to-r from-electric to-orchid px-8 py-4 font-display text-[15px] font-bold text-white shadow-[0_16px_40px_-14px_rgba(182,109,199,0.85)] transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-45 disabled:hover:translate-y-0`}>
            {busy ? "Submitting…" : editing ? "Save changes" : "Submit — mark as done"}
          </button>
          {onCancel && (
            <button type="button" onClick={onCancel}
              className="rounded-2xl border border-white/12 bg-white/[0.05] px-5 py-4 font-display text-[15px] font-bold text-white/60 transition hover:bg-white/[0.1] hover:text-white">
              Cancel
            </button>
          )}
